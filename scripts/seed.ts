...
async function main(): Promise<void> {
  ...
  for (const raw of rawItems) {
    ...
    workItems.push({
      ...
      completion_approval_status: raw.completed_at ? 'APPROVED' : 'NONE',
      submitted_completed_at: raw.completed_at,
      submitted_result_link: raw.result_link,
      completion_submitted_by: raw.completed_at
        ? (raw.assignee_alias ? (userIdByAlias.get(raw.assignee_alias) ?? systemUserId) : systemUserId)
        : null,
      completion_submitted_at: raw.completed_at ? NOW : null,
      completion_reviewed_by: raw.completed_at ? systemUserId : null,
      completion_reviewed_at: raw.completed_at ? NOW : null,
      completion_review_note: raw.completed_at ? 'Dữ liệu hoàn thành được kế thừa từ nguồn seed.' : null,
